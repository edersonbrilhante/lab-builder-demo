...
resource "null_resource" "ansible" {
  ...
  triggers = {
    cmd_id = replace(local.provisioning_command, "$PUBLIC_IP", "'${aws_instance.linux_server.public_ip},'")
  }

  provisioner "local-exec" {
    command = replace(local.provisioning_command, "$PUBLIC_IP", "'${aws_instance.linux_server.public_ip},'")
  }
}
